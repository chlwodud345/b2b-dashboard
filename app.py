...
with tab9:
    ...
    # ── Step 1: 차트 선택 (form으로 감싸서 rerun 억제) ──
    if 'custom_step' not in st.session_state:
        st.session_state.custom_step = 'select'   # 'select' | 'sort' | 'view'
            ...
            for i, (tab_name, charts) in enumerate(tab_list):
                with tab_cols[i % 2]:
                    ...
                    for cid, cname in charts:
                        checked = st.checkbox(
                            cname,
                            value=(cid in st.session_state.custom_selected),
                            key=f"chk_{cid}"
                        )
                        if checked:
    ...
    if st.session_state.custom_step in ('sort', 'view'):
        ...
        if not st.session_state.custom_selected:
            st.warning("선택된 차트가 없습니다. 위에서 차트를 선택해주세요.")
        else:
            ...
            if cid and cid in CHART_REGISTRY:
                ...
                def make_text_input(sfx):
                    ...
                    def _ti(*a, **kw):
                        ...
                        return original_text_input(*a, **kw)
                    return _ti
                ...
                def make_multiselect(sfx):
                    ...
                    def _ms(*a, **kw):
                        ...
                        return original_multiselect(*a, **kw)
                    return _ms
                ...
                def make_selectbox(sfx):
                    ...
                    def _sb(*a, **kw):
                        ...
                        return original_selectbox(*a, **kw)
                    return _sb

                def make_dataframe(sfx):
                    call_count = [0]
                    def _df(*a, **kw):
                        call_count[0] += 1
                        return original_dataframe(*a, **kw)
                    return _df
