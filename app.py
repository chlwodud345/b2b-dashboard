...
def process_data(orders, members, referrals_df):
    ...
    def extract_region(addr):
        ...
        addr = str(addr).strip()
        if addr.startswith(('전라남도','전남','광주','전남광주')): return '전남광주'
        for r in ['서울','부산','대구','인천','대전','울산','세종','경기','강원','충북','충남','전북','경북','경남','제주']:
...
SIDO_NORM = {
    ...
    '광주광역시':'전남광주','광주시':'전남광주','광주':'전남광주','대전광역시':'대전','대전시':'대전','대전':'대전',
    '울산광역시':'울산','울산시':'울산','울산':'울산','세종특별자치시':'세종','세종시':'세종','세종':'세종',
    '경기도':'경기','경기':'경기','강원특별자치도':'강원','강원도':'강원','강원':'강원',
    '충청북도':'충북','충북':'충북','충청남도':'충남','충남':'충남',
    '전북특별자치도':'전북','전라북도':'전북','전북':'전북','전라남도':'전남광주','전남':'전남광주','전남광주통합특별시':'전남광주','전남광주':'전남광주',
...
try:
    ...
    st.sidebar.markdown(f"""<div style="background:#E6F1FB;border:1px solid #B5D4F4;border-radius:8px;padding:8px 12px;font-size:0.8rem;font-weight:600;color:#185FA5;margin-top:-12px;">📅 데이터 기준: {to_date_kr(orders['주문일'].max().strftime('%Y-%m-%d'))}</div>""", unsafe_allow_html=True)
except Exception as e:
...
def render_daily_sales_trend(kp=""):
    ...
    if st.session_state[f"{kp}_daily_expand"]:
        ...
        if view_mode == "일별":
            ...
            monthly_tbl = filtered.groupby('주문월')['판매합계금액'].sum().reset_index()
            monthly_tbl.columns = ['월','매출']
            monthly_tbl['월'] = ym_series_kr(monthly_tbl['월'])
            monthly_tbl = monthly_tbl.sort_values('월', ascending=False).reset_index(drop=True)
            monthly_tbl.loc[len(monthly_tbl)] = ['합계', monthly_tbl['매출'].sum()]
            st.dataframe(monthly_tbl.style.format({'매출':'{:,.0f}원'}),use_container_width=True,height=400)
...
def render_org_sales_table(kp=""):
    ...
    ba = filtered.groupby(['주문자 ID','주문자명','주문자 구분','회원 등급']).agg(
        매출=('판매합계금액','sum'),주문건수=('주문 ID','nunique'),최근주문일=('주문일자','max')
    ).reset_index()
