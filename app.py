...
with tab6:
    ...
    fig = make_subplots(specs=[[{"secondary_y":True}]])
    fig.add_trace(go.Bar(x=cga['등급'],y=cga['매출'],name='매출액',marker_color='#3366CC',opacity=0.8,text=[fmt_krw_short(v) for v in cga['매출']],textposition='outside',textfont=dict(size=10),hovertemplate='%{x}<br>매출: %{customdata}<extra></extra>',customdata=[f"{v:,.0f}원" for v in cga['매출']]),secondary_y=False)
    fig.add_trace(go.Bar(x=cga['등급'],y=cga['주문건수'],name='주문건수',marker_color='#E8853D',opacity=0.8,text=[fmt_num(v) for v in cga['주문건수']],textposition='outside',textfont=dict(size=10),hovertemplate='%{x}<br>주문: %{y:,}건<extra></extra>'),secondary_y=True)
    fig.update_layout(height=480,barmode='group',margin=dict(l=70,r=60,t=50,b=40),legend=dict(orientation="h",yanchor="bottom",y=1.02,x=0,font=dict(size=11)),xaxis=dict(title='',tickfont=dict(size=13),categoryorder='array',categoryarray=grade_order))
    fig.update_yaxes(title_text="매출액",tickvals=tvals_cf,ticktext=ttexts_cf,tickfont=dict(size=11),secondary_y=False); fig.update_yaxes(title_text="주문건수",tickfont=dict(size=11),secondary_y=True)
    st.plotly_chart(fig, use_container_width=True)
    ...
    fig.update_layout(height=450,margin=dict(l=70,r=30,t=30,b=60),showlegend=False,xaxis=dict(title='',tickfont=dict(size=12)),yaxis=dict(title='매출액',tickfont=dict(size=11)))
    st.plotly_chart(fig, use_container_width=True)
