...
with tab6:
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(name='매출액',x=[list(cga['등급']),['매출']*len(cga)],y=cga['매출'],marker_color='#3366CC',text=[fmt_krw_short(v) for v in cga['매출']],textposition='outside',textfont=dict(size=10),hovertemplate='%{x[0]}<br>매출: %{customdata}<extra></extra>',customdata=[f"{v:,.0f}원" for v in cga['매출']]))
    fig.add_trace(go.Bar(name='주문건수',x=[list(cga['등급']),['주문']*len(cga)],y=cga['주문건수'],marker_color='#E8853D',text=[fmt_num(v) for v in cga['주문건수']],textposition='outside',textfont=dict(size=10),hovertemplate='%{x[0]}<br>주문: %{y:,}건<extra></extra>'))
