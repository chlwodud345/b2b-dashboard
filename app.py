...
def krw_tick(val, _=None):
    """plotly y축 tick용"""
    if abs(val) >= 1e8: return f"{val/1e8:.0f}억"
    if abs(val) >= 1e4: return f"{val/1e4:.0f}만"
    return f"{val:,.0f}"

def group_small_pie(df, name_col, value_col, threshold_pct=3):
    """도넛차트에서 비중 작은 항목을 '기타'로 합침"""
    total = df[value_col].sum()
    if total == 0: return df
    df = df.copy()
    df['_pct'] = df[value_col] / total * 100
    big = df[df['_pct'] >= threshold_pct].copy()
    small = df[df['_pct'] < threshold_pct]
    if len(small) > 0:
        etc = pd.DataFrame({name_col: ['기타'], value_col: [small[value_col].sum()]})
        big = pd.concat([big, etc], ignore_index=True)
    return big[[name_col, value_col]]
...
with tab1:
    ...
    to = filtered['주문 ID'].nunique()
    tb = filtered['주문자 ID'].nunique()
    tm = len(members)
    ao = ts/to if to>0 else 0
    
    cols = st.columns(5)
    for col,(l,v,u) in zip(cols,[
        ("총 매출액",fmt_krw_short(ts),"원"),("총 주문건수",fmt_num(to),"건"),
        ("총 회원수",fmt_num(tm),"처"),("주문회원수",fmt_num(tb),"처"),("객단가",fmt_krw_short(ao),"원"),
    ]):
    ...
    fig.add_trace(go.Bar(x=monthly['주문월'],y=monthly['매출'],name='매출액',marker_color='#3366CC',opacity=0.8,
                         hovertemplate='%{x}<br>매출: %{customdata}<extra></extra>',
                         customdata=[fmt_krw(v) for v in monthly['매출']]),secondary_y=False)
    fig.add_trace(go.Scatter(x=monthly['주문월'],y=monthly['주문건수'],name='주문건수',
                             line=dict(color='#E8853D',width=2.5),mode='lines+markers',
                             hovertemplate='%{x}<br>주문: %{y:,}건<extra></extra>'),secondary_y=True)
    fig.update_layout(height=420,margin=dict(l=60,r=50,t=70,b=50),
                      title=dict(text='월별 매출 · 주문건수 추이',x=0.01,font=dict(size=15)),
                      legend=dict(orientation="h",yanchor="bottom",y=1.02,x=0,font=dict(size=11)))
    fig.update_yaxes(title_text="매출액",tickvals=monthly['매출'].quantile([0,0.25,0.5,0.75,1]).tolist() if len(monthly)>0 else [],
                     ticktext=[krw_tick(v) for v in (monthly['매출'].quantile([0,0.25,0.5,0.75,1]).tolist() if len(monthly)>0 else [])],
                     secondary_y=False)
    ...
    with cl:
        ...
        ts_df.columns = ['구분','매출']
        ts_df = group_small_pie(ts_df, '구분', '매출', threshold_pct=3)
    ...
    with cr:
        ...
        fig.update_layout(height=420,title=dict(text='지역별 매출',x=0.01,font=dict(size=15)),
                          ...
                          xaxis=dict(tickvals=rg['매출'].quantile([0,0.5,1]).tolist(),
                                     ticktext=[krw_tick(v) for v in rg['매출'].quantile([0,0.5,1]).tolist()]))
        st.plotly_chart(fig, use_container_width=True)
...
with tab2:
    ...
    fig = px.bar(tm_df,x='주문월',y='판매합계금액',color='주문자 구분',color_discrete_sequence=COLORS)
    fig.update_traces(hovertemplate='%{x}<br>%{data.name}: %{customdata}<extra></extra>')
    # 커스텀데이터 각 trace에 설정
    for trace in fig.data:
        trace.customdata = [fmt_krw(v) for v in trace.y]
        trace.hovertemplate = '%{x}<br>' + trace.name + ': %{customdata}<extra></extra>'
    ...
    gs = filtered.groupby('회원 등급')['판매합계금액'].sum().sort_values().reset_index()
    fig = px.bar(gs,x='판매합계금액',y='회원 등급',orientation='h',color_discrete_sequence=COLORS)
    fig.update_traces(hovertemplate='%{y}: %{customdata}<extra></extra>',
                      customdata=[fmt_krw(v) for v in gs['판매합계금액']])
...
with tab4:
    ...
    cols = st.columns(5)
    for col,(l,v,u) in zip(cols,[
        ("총 회원수",fmt_num(len(members)),"처"),("구매전환율",fmt_pct(conv_r),""),
        ("재구매율",fmt_pct(rep_r),""),("활성회원",fmt_num(act),"처"),
        ("총 주문회원",fmt_num(len(mo_df)),"처"),
    ]):
        col.markdown(kpi_card(l,v,u),unsafe_allow_html=True)
    ...
    cl,cr = st.columns(2)
    with cl:
        mg = members.merge(mo_df,left_on='아이디',right_on='주문자 ID',how='inner')
    ...
    with cr:
        ...
        mo_df['구간'] = pd.cut(mo_df['주문건수'],bins=bo,labels=lo,right=False)
        od = mo_df['구간'].value_counts().reindex(lo).fillna(0).reset_index()
        od.columns=['구간','회원수']
...
with tab5:
    ...
    with cl:
        ...
        st.plotly_chart(fig, use_container_width=True)
    with cr:
        ts_df=rdf.groupby('유형')['피추천인매출'].sum().reset_index()
        ts_df = group_small_pie(ts_df,'유형','피추천인매출',threshold_pct=1)
        fig=px.pie(ts_df,values='피추천인매출',names='유형',hole=0.5,color='유형',color_discrete_map=tc)
        fig.update_traces(hovertemplate='%{label}<br>매출: %{customdata}<br>비중: %{percent}<extra></extra>',
                          customdata=[fmt_krw(v) for v in ts_df['피추천인매출']])
