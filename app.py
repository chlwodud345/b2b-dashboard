...
with tab6:
    ...
    fig=make_subplots(specs=[[{"secondary_y":True}]])
    fig.add_trace(go.Bar(x=cga['등급'],y=cga['매출'],name='매출액',marker_color='#3366CC',opacity=0.8,text=[fmt_krw_short(v) for v in cga['매출']],textposition='outside',textfont=dict(size=10),hovertemplate='%{x}<br>매출: %{customdata}<extra></extra>',customdata=[fmt_krw(v) for v in cga['매출']]),secondary_y=False)
    fig.add_trace(go.Scatter(x=cga['등급'],y=cga['주문건수'],name='주문건수',line=dict(color='#E8853D',width=3),mode='lines+markers',marker=dict(size=8),hovertemplate='%{x}<br>주문: %{y:,}건<extra></extra>'),secondary_y=True)
    fig.update_layout(height=480,margin=dict(l=70,r=60,t=50,b=40),legend=dict(orientation="h",yanchor="bottom",y=1.02,x=0,font=dict(size=11)),xaxis=dict(title='',tickfont=dict(size=13)))
    fig.update_yaxes(title_text="매출액",tickfont=dict(size=11),secondary_y=False); fig.update_yaxes(title_text="주문건수",tickfont=dict(size=11),secondary_y=True)
    st.plotly_chart(fig, use_container_width=True)
