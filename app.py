...
def match_pilot_clinics(pilot_df, members_df, orders_df, similarity_threshold=60):
    ...
    for idx, row in pilot_df.iterrows():
        ...
        if bid and bid in mem_by_id.index:
            m = mem_by_id.loc[bid]
            results.append({'기관명': row['기관명'], '기관구분': row.get('기관구분', ''), '사업유형': row['사업유형'],
    ...
    for _, m in mem[mem['전화번호_norm'].str.len() >= 9].iterrows():
        phone_map.setdefault(m['전화번호_norm'], m)
    for idx, row in pilot_df.iterrows():
        phone = row['전화번호_norm']
    ...
    for _, m in mem.iterrows():
        ...
        if key not in mem_region_name:
            mem_region_name[key] = m
    for idx, row in pilot_df.iterrows():
    ...
    for idx, row in pilot_df.iterrows():
        ...
        if best_score >= 85 and best_m is not None:
            results.append({'기관명': row['기관명'], '기관구분': row.get('기관구분', ''), '사업유형': row['사업유형'],
                ...
                '매칭방법': f'유사도매칭({best_score}%)', '매칭등급': '확정', '유사도': best_score})
            matched_idx.add(idx)
    if not results:
        return pd.DataFrame()
...
if os.path.exists(logo_path):
    ...
    st.markdown(f'<div class="main-header" style="display:flex;align-items:center;justify-content:space-between;"><div><h1>📊 B2B몰 대시보드 구축 프로젝트5</h1><p>Sales & Operations Analytics</p></div><img src="data:image/png;base64,{logo_b64}" style="height:50px;object-fit:contain;"></div>', unsafe_allow_html=True)
else:
    st.markdown('<div class="main-header"><h1>📊 B2B몰 대시보드 구축 프로젝트5</h1><p>Sales & Operations Analytics</p></div>', unsafe_allow_html=True)

# ============================================================
# 차트 렌더링 함수 모음 (커스터마이징 탭용)
# ============================================================

def render_monthly_sales_trend():
...
def render_member_type_sales_donut():
...
def render_org_sales_table():
    ...
    search = st.text_input("🔍 검색 (아이디, 주문자명, 상호명)",key="custom_sales_search")
...
def render_product_sales_table():
    st.markdown("#### 전체 상품 매출 현황")
    pa = filtered.groupby(['상품명','상품 코드']).agg(매출=('판매합계금액','sum'),수량=('주문 수량','sum'),주문건수=('주문 ID','nunique')).reset_index().sort_values('매출',ascending=False)
    sp = st.text_input("🔍 상품명/코드 검색",key="custom_product_search"); dp = pa.copy().reset_index(drop=True)
...
def render_product_monthly_trend():
    st.markdown("#### 월별 상품 매출 추이")
    pa = filtered.groupby(['상품명','상품 코드']).agg(매출=('판매합계금액','sum')).reset_index().sort_values('매출',ascending=False)
    t5 = pa.head(5)['상품명'].tolist(); sel = st.multiselect("상품 선택",pa['상품명'].tolist(),default=t5,key="custom_product_trend")
    if sel:
    ...
    if rd:
        ...
        for _, r in rdf.iterrows():
            ...
            for offset in range(mx):
                ...
                row_hover.append(f"{offset}개월 후 ({actual.year}년 {actual.month}월)")
...
def render_dormant_analysis():
    ...
    def classify_status(row):
        ...
        days = row['휴면경과일']
        if days <= 90: return '활성'
        elif days <= 180: return '단기휴면'
        elif days <= 365: return '중기휴면'
        else: return '장기휴면'
    ...
    with c2:
        fig = go.Figure()
        fig.add_trace(go.Bar(x=status_counts['구분'], y=status_counts['수'], marker_color=[status_colors[s] for s in status_counts['구분']],
            text=[fmt_num(v) for v in status_counts['수']], textposition='outside', textfont=dict(size=12), hovertemplate='%{x}: %{y:,}처<extra></extra>'))
        fig.update_layout(height=450, showlegend=False, margin=dict(l=60, r=30, t=30, b=40),
            xaxis=dict(title='', tickfont=dict(size=12), categoryorder='array', categoryarray=status_order),
            yaxis=dict(title='회원 수 (처)', tickfont=dict(size=11)))
        st.plotly_chart(fig, use_container_width=True)

def render_referral_count_bar():
    ...
    for _,r in referrals_df.iterrows():
        ...
        if not n or n in ['-','nan']: continue
        if n not in ra: ra[n]={'추천인':n,'유형':clm.get(n,'케어포'),'피추천인수':0,'biz':[]}
        b=str(r.get('피추천인 사업자 번호','')).strip()
        if b and b not in ['-','nan']: ra[n]['피추천인수']+=1; ra[n]['biz'].append(b)
    rdf_local=pd.DataFrame(ra.values())[['추천인','유형','피추천인수']]
...
def render_referral_sales_donut():
...
def render_referral_table():
    ...
    b2u=members.set_index('사업자번호')['아이디'].to_dict(); bs=filtered.groupby('주문자 ID')['판매합계금액'].sum().to_dict()
    for n in ra: ra[n]['피추천인매출']=sum(bs.get(b2u.get(b,''),0) for b in ra[n]['biz'])
    rdf_local=pd.DataFrame(ra.values())[['추천인','유형','추천인코드','피추천인수','피추천인매출']]
    rdf_local=rdf_local.sort_values('피추천인매출',ascending=False).reset_index(drop=True)
    sr=st.text_input("🔍 추천인 검색",key="custom_ref_search")
...
def render_carefor_grade_sales():
    ...
    fig.update_layout(height=420,margin=dict(l=60,r=20,t=30,b=40),showlegend=False,xaxis=dict(title='',tickfont=dict(size=12),categoryorder='array',categoryarray=grade_order),yaxis=dict(title='매출액',tickfont=dict(size=11)))
    st.plotly_chart(fig, use_container_width=True)

def render_carefor_grade_orders():
...
def render_carefor_monthly_trend():
...
def render_carefor_new_member_trend():
...
def render_pnl_waterfall():
    ...
    fig = go.Figure(go.Waterfall(x=['매출액','매출원가','매출총이익','판관비','영업이익'],y=wf_values,measure=['absolute','relative','total','relative','total'],
        connector=dict(line=dict(color="#94a3b8",width=1)),increasing=dict(marker=dict(color='#27AE60')),decreasing=dict(marker=dict(color='#E74C3C')),totals=dict(marker=dict(color='#3366CC')),
        text=[fmt_krw_short(abs(v)) for v in wf_values],textposition='outside',textfont=dict(size=12),hovertemplate='%{x}<br>금액: %{customdata}<extra></extra>',customdata=[f"{abs(v):,.0f}원" for v in wf_values]))
    wf_tvals, wf_ttexts = krw_tickvals(pd.Series([abs(v) for v in wf_values]))
    fig.update_layout(height=480,margin=dict(l=80,r=30,t=50,b=60),xaxis=dict(tickfont=dict(size=13)),yaxis=dict(title='금액',tickvals=wf_tvals,ticktext=wf_ttexts,tickfont=dict(size=11)),showlegend=False)
    st.plotly_chart(fig, use_container_width=True)

def render_pnl_monthly_trend():
...
def render_channel_pnl():
    ...
    fig.update_layout(height=max(450,len(ch_pnl)*50+140),barmode='group',margin=dict(l=130,r=100,t=30,b=40),legend=dict(orientation="h",yanchor="bottom",y=1.02,x=0,font=dict(size=11)),xaxis=dict(title='금액',tickvals=ch_tvals,ticktext=ch_ttexts,tickfont=dict(size=11)),yaxis=dict(title='',tickfont=dict(size=11)))
    st.plotly_chart(fig, use_container_width=True)

def render_channel_pnl_table():
...
def render_product_pnl_hierarchy():
    st.markdown("#### 제품계층구조별 수익성 분석")
    if len(bw_filtered) == 0: st.warning("BW 데이터 없음"); return
    bw = bw_filtered.copy()
    def _render(df, group_col, tab_key):
    ...
    with sub1: _render(bw, '제품계층구조(대)', 'c_large')
    with sub2:
        sel = st.selectbox("대분류 선택",["전체"]+sorted(bw['제품계층구조(대)'].unique().tolist()),key="c_mid_filter")
        _render(bw if sel=="전체" else bw[bw['제품계층구조(대)']==sel], '제품계층구조(중)', 'c_medium')
    with sub3:
        c1, c2 = st.columns(2)
        with c1: sel_l = st.selectbox("대분류",["전체"]+sorted(bw['제품계층구조(대)'].unique().tolist()),key="c_small_l")
        bw_s = bw if sel_l=="전체" else bw[bw['제품계층구조(대)']==sel_l]
        with c2: sel_m = st.selectbox("중분류",["전체"]+sorted(bw_s['제품계층구조(중)'].unique().tolist()),key="c_small_m")
        _render(bw_s if sel_m=="전체" else bw_s[bw_s['제품계층구조(중)']==sel_m], '제품계층구조(소)', 'c_small')

def render_material_pnl_table():
    ...
    ms = st.text_input("🔍 자재명/코드 검색",key="custom_mat_search")
...
def render_pilot_type_bar(pilot_df, match_df):
    ...
    type_summary['가입수'] = type_summary['가입수'].fillna(0).astype(int)
    type_summary['미가입'] = type_summary['기관수'] - type_summary['가입수']
    fig = go.Figure()
...
def render_pilot_region_bar(pilot_df, match_df):
    ...
    region_all['미가입'] = region_all['시범기관수'] - region_all['가입기관수']
    region_all = region_all.sort_values('시범기관수')
    fig = go.Figure()
...
def render_pilot_top20(pilot_df, match_df):
    ...
    if len(top_dedup) == 0: st.info("매출 발생 기관이 없습니다."); return
...
def render_pilot_unmatched_table(pilot_df, match_df):
    ...
    if not match_df.empty:
        ...
        unmatched = pilot_df[['사업유형','기관명','기관구분','주소','전화번호']].reset_index(drop=True)
    search_un = st.text_input("🔍 미매칭 기관 검색",key="custom_unmatched_search")
    if search_un: unmatched = unmatched[unmatched.apply(lambda r:search_un.lower() in str(r).lower(),axis=1)]
...
with tab4:
    ...
    with col_filter: status_filter = st.multiselect("활성 구분 필터",status_order,default=[],placeholder="전체",key="dormant_status_filter")
    with col_type:
    ...
    display_cols = ['추천인','상호명','아이디','휴대폰','주소','가입일','회원타입','회원등급','마지막주문일','휴면경과일','SMS 수신동의','활성구분']
    ...
    st.download_button(label="📥 엑셀 다운로드",data=excel_data,file_name=f"휴면회원분석_{pd.Timestamp.now().strftime('%Y%m%d')}.xlsx",mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
...
with tab7:
    if len(bw_data) == 0:
        ...
        def render_product_pnl(df, group_col, tab_key):
            ...
            ps = st.text_input(f"🔍 {group_col} 검색",key=f"prod_search_{tab_key}")
            if ps: tbl = tbl[tbl[group_col].str.contains(ps,case=False,na=False)]
            st.dataframe(tbl.style.format({'매출액':'{:,.0f}원','매출총이익':'{:,.0f}원','매출총이익률':'{:.1f}%','영업이익':'{:,.0f}원','영업이익률':'{:.1f}%'}),use_container_width=True,height=400)
        with prod_sub1: render_product_pnl(bw_prod,'제품계층구조(대)','large')
        with prod_sub2:
            sel_large = st.selectbox("대분류 선택",["전체"]+sorted(bw_prod['제품계층구조(대)'].unique().tolist()),key="bw_mid_filter")
            render_product_pnl(bw_prod if sel_large=="전체" else bw_prod[bw_prod['제품계층구조(대)']==sel_large],'제품계층구조(중)','medium')
        with prod_sub3:
            c1,c2=st.columns(2)
            with c1: sel_large2 = st.selectbox("대분류 선택",["전체"]+sorted(bw_prod['제품계층구조(대)'].unique().tolist()),key="bw_small_filter_l")
            bw_small = bw_prod if sel_large2=="전체" else bw_prod[bw_prod['제품계층구조(대)']==sel_large2]
            with c2: sel_mid = st.selectbox("중분류 선택",["전체"]+sorted(bw_small['제품계층구조(중)'].unique().tolist()),key="bw_small_filter_m")
            render_product_pnl(bw_small if sel_mid=="전체" else bw_small[bw_small['제품계층구조(중)']==sel_mid],'제품계층구조(소)','small')
...
with tab9:
    ...
    st.markdown("원하는 차트를 선택하고, 드래그앤드롭으로 순서를 조정한 뒤 **▶ 대시보드 보기** 버튼을 누르세요.")

    # 탭별 차트 그룹화
    tab_groups = {}
    for cid, info in CHART_REGISTRY.items():
        tab_groups.setdefault(info["tab"], []).append((cid, info["name"]))

    # 선택 UI (탭별로 expander)
    if 'custom_selected' not in st.session_state:
        st.session_state.custom_selected = []

    with st.expander("📋 차트 선택", expanded=True):
        new_selected = list(st.session_state.custom_selected)
        ...
        for i, (tab_name, charts) in enumerate(tab_list):
            with tab_cols[i % 2]:
                ...
                for cid, cname in charts:
                    checked = st.checkbox(cname, value=(cid in new_selected), key=f"chk_{cid}")
                    if checked and cid not in new_selected:
                        new_selected.append(cid)
                    elif not checked and cid in new_selected:
                        new_selected.remove(cid)
        st.session_state.custom_selected = new_selected

    st.markdown(f"**선택된 차트: {len(st.session_state.custom_selected)}개**")

    if not st.session_state.custom_selected:
        st.info("위에서 차트를 선택하면 순서 조정 및 미리보기가 가능합니다.")
    else:
        st.markdown("---")
        st.markdown("#### 🔀 순서 조정 (드래그앤드롭)")

        # sortables에 표시할 레이블 리스트
        sorted_labels = [CHART_REGISTRY[cid]["name"] for cid in st.session_state.custom_selected]

        sorted_result = sort_items(sorted_labels, direction="vertical", key="custom_sort")
        ...
        if st.button("▶ 대시보드 보기", type="primary", use_container_width=True):
            st.session_state.custom_run = sorted_result

        if st.session_state.get("custom_run"):
            st.markdown("---")
            st.markdown("## 📊 커스텀 대시보드")
            for label in st.session_state.custom_run:
